...
void MsgSndISR(int mid, msg_t* msg){
   msg->sender = cur_pid;
   msg->send_tick = sys_tick;
   
   if(EmptyQ(&mboxes[mid].wait_q)){
      MsgEnQ(msg, &mboxes[mid].msg_q);
   }
   else{
      EnQ(DeQ(&mboxes[mid].wait_q), &ready_q);
      pcbs[mid].state = READY;
      MyMemCpy((char*)pcbs[mid].tf_p->ebx, (char*)msg, sizeof(msg_t));
   }
}

void MsgRcvISR(int mid, msg_t* msg){
   if(!MsgEmptyQ(&mboxes[mid].msg_q)){
   
     MyMemCpy((char*) msg, (char*) MsgDeQ(&mboxes[mid].msg_q), sizeof(msg_t));
   }
   else {

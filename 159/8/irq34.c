...
void Stdout() {
   msg_t msg;
   int term_num, sender_pid;

   MsgRcv(&msg); // msg provided by Init() for this process to know:

   term_num = msg.nums[1];  // need info which terminal structure to use
   sender_pid = msg.sender;  // need to know mbox ID of shell servicing

   while(1) { // service loop, servicing shell_pid only actually
      MsgRcv(&msg);
      sender_pid = msg.sender;  // need to know mbox ID of shell servicing
      
      if(msg.nums[4] == ECHO_OFF){
		  terminals[term_num].echo_on = 0;
      }
      else{
         terminals[term_num].echo_on = 1;
      }      
      StdoutString(term_num, msg.bytes); // bytes is str to display
      MsgSnd(sender_pid, &msg);                // completion msg, content not important
   }
}
...
void Stdin() {
   ...
   while(1) // loop of service, servicing shell_pid only actually
   {
      ...
      if(*msg.bytes == '\0') MyStrCpy(msg.bytes, "\n");
